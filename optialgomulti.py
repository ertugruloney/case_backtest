...
if __name__ == "__main__":
    ...
    dosya_yolu = 'coins/*.csv'  # CSV dosyalarının bulunduğu dizin

    # Glob ile dosya isimlerini bul
    csv_files = glob.glob(dosya_yolu)
    ...
    #%%
        ...
        Re=strategy.run_strategy(df,result_df.values)
        total_profit = Re['Profit'].sum()
        dictt={
            "symbol":symbol,
            "Profit":total_profit}
        Trades_profit.append(dictt)
...
#%%
    Trades_profit=pd.DataFrame(Trades_profit)
    Trades_profit.to_csv("resultmulti.csv", index=False)
